          ...
          <div key={category} className="category-section">
            ...
            <div className="Program-cards">
              {categorizedPrograms[category].map((program, index) => {
                ...
                return (
                  <div key={uniqueKey} className={`program-card ${openPrograms[uniqueKey] ? "active" : ""}`} onClick={() => toggleProgram(category, index)}>
                    <div className="program-header">
                      <h3>{program.name}</h3>
                      <span className="toggle-icon">{openPrograms[uniqueKey] ? "-" : "+"}</span>
                    </div>
